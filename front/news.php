<div class="di"
    ...
    <marquee scrolldelay="120" direction="left" style="position:absolute; width:90%; height:40px;">
        <?php
        $ads = $Ad->all(['sh' => 1]);
        foreach ($ads as $ad) {
            echo $ad['text'];
    ...
    <div style="height:32px; text-align:center; display:block;">
    </div>
    ...
    <div style="width:99%; height:50%; position:relative;" class="cent">
        <?php
            $mvims = $Mvim->all(['sh' => 1]);
            foreach ($mvims as $mvim) {
                echo "<img style='height:65%' src='upload/{$mvim['img']}'>";
            };
        ?>
    </div>
